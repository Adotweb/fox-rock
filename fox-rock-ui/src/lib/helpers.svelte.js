...
export function create_chunked_map(map){
	...
	let chunked_map = Array.from({ length : map_side_length/8 }, () => Array.from({length : map_side_length/8}, () => 
		new Array(64).fill(0)
	)) 
	...
	map.forEach(({cell_state, index}) => {
		...
		if(cell_state == 1){
			chunked_map[chunk_pos[1]][chunk_pos[0]][chunk_coords[0] * 8 + chunk_coords[1]] = cell_state
		}
